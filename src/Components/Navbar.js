    ...
    <>
      <div className={`${bgColor} bg-opacity-30 transition-all z-50 sticky top-0 flex justify-between items-center pl-4 py-4 lg:px-4 lg:pl-5`}>
        <li className="text-2xl ml-2 hover:scale-105 transition-all list-none sm:text-3xl sm:ml-4 md:text-4xl md:ml-5 lg:text-5xl lg:ml-6">
          <Link className={`${webMode === "Light" ? "text-DarkColor" : "text-gray-300"}  active font-kanit font-bold`} aria-current="page" to="/Purvish_Portfolio">
            {NavBarText}
          </Link>
        </li> 
        <div className="flex justify-between items-center">
          ...
          <CustomizedSwitches toggleMode={toggleMode} webMode={webMode} className="" /> {/* ✅ Pass toggleMode */}
        </div>
        <div ref={navRef} className={`absolute top-28 right-1 transition-all text-center flex-col font-bold z-10`}>
          {navBarContentMiddle.map((item, index) => (
            ...
            >
              <Link className={`${webMode === "Light" ? "text-white" : "text-black"} active font-Karla transition-all`} to={item.path} onClick={() => setNavBarText(item.name)}>
                {item.name}
              </Link>
            </li>
          ))}
